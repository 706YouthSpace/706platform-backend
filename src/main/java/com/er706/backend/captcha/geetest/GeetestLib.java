package com.er706.backend.captcha.geetest;
...
public class GeetestLib {
  ...
  private String requestRegister(Map<String, String> paramMap) {
    ...
    this.gtlog(String
        .format("requestRegister(): 验证初始化, 向极验发送请求, url=%s, params=%s.", register_url, paramMap));
   ...
   * 正常流程下（即验证初始化成功），二次验证
   */
  public GeetestLibResult successValidate(String challenge, String validate, String seccode,
      Map<String, String> paramMap) {
    this.gtlog(String
        .format("successValidate(): 开始二次验证 正常模式, challenge=%s, validate=%s, seccode=%s.", challenge,
            validate, seccode));
  ...
  public GeetestLibResult failValidate(String challenge, String validate, String seccode) {
    this.gtlog(String
        .format("failValidate(): 开始二次验证 宕机模式, challenge=%s, validate=%s, seccode=%s.", challenge,
            validate, seccode));
    if (!this.checkParam(challenge, validate, seccode)) {
   ...
   * 向极验发送二次验证的请求，POST方式
   */
  private String requestValidate(String challenge, String validate, String seccode,
      Map<String, String> paramMap) {
    ...
    this.gtlog(String
        .format("requestValidate(): 二次验证 正常模式, 向极验发送请求, url=%s, params=%s.", validate_url,
            paramMap));
  ...
  private boolean checkParam(String challenge, String validate, String seccode) {
    return !(challenge == null || challenge.trim().isEmpty() || validate == null || validate.trim()
        .isEmpty() || seccode == null || seccode.trim().isEmpty());
  }
  ...
  private String httpGet(String url, Map<String, String> paramMap) throws Exception {
    ...
    try {
      ...
      while (it.hasNext()) {
        String key = it.next();
        if (key == null || key.isEmpty() || paramMap.get(key) == null || paramMap.get(key)
            .isEmpty()) {
          continue;
        }
        paramStr.append("&").append(URLEncoder.encode(key, "utf-8")).append("=")
